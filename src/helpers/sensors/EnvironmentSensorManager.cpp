...
// ------------------------------------------------------------
// Kleiner I2C-Helper: liest ein 16-bit Register (big-endian),
// wie es INA226/INA260/INA3221 für ihre ID-Register nutzen.
// Gibt false zurück, wenn der Bus-Transfer fehlschlägt.
// ------------------------------------------------------------
...
// Liest Manufacturer-ID (0xFE) + Die-ID (0xFF) und liefert die oberen 12 Bit
// der Die-ID zurück (die unteren 4 Bit sind nur eine Revisionsnummer).
// Gibt 0 zurück, wenn keine gültige TI-ID lesbar ist (z.B. bei INA219).
...
static uint8_t init_ina226(TwoWire* wire, uint8_t addr) {
  ...
  if (identify_ina_chip(wire, addr) != TELEM_INA226_ID) return 0;
